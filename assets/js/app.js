...
$(function() {
    ...
    $(document).on('mouseenter click touchstart', productsCardSelector, function(e) {
        $(document).find($(productsCardSelector)).removeClass('raise');
        $(this).addClass('raise');
    });

    $(document).on('mouseleave', productsCardSelector, function(e) {
        $(document).find($(productsCardSelector)).removeClass('raise');
    });

    var productsSliders = $('.products-slider');
    productsSliders.find('[class^=col-]').removeClass();
    if(productsSliders.length) {
        const fixMargin = function() {
            $(productsCardSelector).removeClass('raise');
        }
        ...
        for(const mutation of mutationsList) {
            if (mutation.type === 'attributes') {
                $('.tns-ovh').each((_, tns) => {
                    var raise = $(tns).find(`${productsCardSelector}.raise`);
                    if(raise.length) {
                        var target = $(tns).find(`${productsCardSelector}.raise`);
                        ...
                        var target = $(tns).find(productsCardSelector)[0];
                        var height = $(target).height();
