...
class SettingsTranslationFileController extends SiteCacheController
{
    ...
    /**
     * @Route("/getSettingsTranslationFile", name="frontoffice_get_api_settings_translation_file_get", methods={"GET"})
     */
    public function getSettingsTranslationFileGet()
    {
        $objLayout = new Layout($this->em, $this->params, $this->requestStack, $this->session, $this->objSettingsService);

        if (!$objLayout->isDev($this->request)) {
            dd('This action has to be execute in Dev Environment');
        }

        return $this->getSettingsTranslationFilePost();
    }

    /**
     * @Route("/getSettingsTranslationFile", name="frontoffice_get_api_settings_translation_file_post", methods={"POST"})
     */
    public function getSettingsTranslationFilePost()
    {
        ...
        foreach ($arContent AS $content) {
            ...
            if ($isMessage) {
                ...
                if (!array_key_exists($referenceKey, $arRet[$isMessage][$filename][$translationCode])) {
                    $arRet[$isMessage][$filename][$translationCode][$referenceKey] = $text;
                }
                ...
                if (!array_key_exists($translationCode, $arRet[$isMessage][$filename][$referenceKey])) {
                    $arRet[$isMessage][$filename][$referenceKey][$translationCode] = $text;
                }
        ...
        foreach ($arContent AS $i => $line) {
            ...
            if ($i > 2 && $i + 3 < count($arContent)) {
                ...
                */
                ...
                $col03 = trim($col03);
                $text = htmlspecialchars($col03);

                if (!in_array($text, $arMessage)) {
                    $sql = 'INSERT INTO `settings_translation_file_content` (id, reference_key, settings_translation_file_id, active) VALUES (' . $dbid . ', "' . $text . '", ' . $fileId . ', 1);';
                    echo $sql . '<br>';
                    foreach ($arTrans AS $transId) {
                        $sql = 'INSERT INTO `settings_translation_file_content_translation` (settings_translation_file_content_id, translation_id, text) VALUES (' . $dbid . ', ' . $transId . ', "");';
                        echo $sql . '<br>';
                    }
                    echo '<br>';
                    $dbid++;
