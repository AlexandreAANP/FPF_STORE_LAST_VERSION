...
class DefaultController extends SiteCacheController
{
    ...
    public function index(Request $request)
    {
        ...
        $subBanners = [];
        $url = $this->apiUrl . '/api/content?type=richmedia&area=content-area-subbanners&area=content-area-store&fields=url&category=richmedia-category-subbanners&language=' . $defaultLanguage;
        if ($data = $this->getAPIData($url)) {
            if ($objData = json_decode($data, JSON_UNESCAPED_UNICODE)) {
                if (array_key_exists('colContent', $objData)) {
                    $subBanners = $objData['colContent'];
                }
            }
        }
