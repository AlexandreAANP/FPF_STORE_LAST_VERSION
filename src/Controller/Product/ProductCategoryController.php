...
class ProductCategoryController extends SiteCacheController
{
    ...
    public static function getCategories(SiteCacheController $parent, $productType, $productCategoryId, $defaultLanguage, $options = [])
    {
        $search = '';
        if ($options && array_key_exists('search', $options)) {
            $search = $options['search'];
        }

        $region = '';
        if ($options && array_key_exists('region', $options)) {
