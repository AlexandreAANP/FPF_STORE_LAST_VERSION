...
use App\Controller\Product\ProductCategoryController;
...
Class Layout extends AbstractController {
    ...
    function getCurrencySymbol() {
        return $this->getEnvVars('DEFAULT_CURRENCY_SYMBOL');
    }

    function getCurrencyCode() {
        return $this->getEnvVars('DEFAULT_CURRENCY_CODE');
    }

    public function getProductCategories() {
        $objSiteCacheController = new SiteCacheController($this->em, $this->params, $this->requestStack, $this->session, $this->objSettingsService);

        $defaultLanguage = $this->request->getLocale();
        $colProductCategories = [];
        if ($objData = ProductCategoryController::getCategories($objSiteCacheController, '', 0, $defaultLanguage, [])) {
            if (array_key_exists('colProductCategories', $objData)) {
                $this->session->set('product_categories_all', $objData);
    ...
    public function getMoneyFormat($number){

        $currentCurrency = 'EUR';
        //switch ($this->getCurrencySymbol()) {
        switch ($currentCurrency) {
            ...
            case 'JPY':
                $fmt = new \NumberFormatter( 'ja-JP', \NumberFormatter::CURRENCY );
            break;break;
            default:
                $fmt = new \NumberFormatter( 'pt_PT', \NumberFormatter::CURRENCY );
            break;
        }
        return $fmt->formatCurrency((float)($number), $currentCurrency);
    }
